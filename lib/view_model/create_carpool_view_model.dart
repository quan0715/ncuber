...
class CreateCarPoolViewModel extends ChangeNotifier {
  ...
  bool get canDrawRoute =>
      mapApi.startPointLatLng != null && mapApi.destinationLatLng != null;
  ...
  MapRoute? mapRoute =
      MapRoute(duration: const Duration(seconds: 0), distance: 0, points: []);
  bool showButtomSheet = false;

  // car model
  DateTime get startTime => carModel.startTime ?? DateTime.now();
  DateTime get endTime =>
      startTime.add(mapRoute?.duration ?? const Duration(seconds: 0));
  ...
  List<String> genderConstrainLabel = const ["性別不拘", "限男性", "限女性"];
  ...
  int genderConstrainDropdownMenuIndex = 0;
  String getTimeString(DateTime time) {
    final nowDate = checkFormatter.format(DateTime.now());
    return nowDate == checkFormatter.format(time)
  ...
  String? roomTitleValidator(String? value) =>
      (roomTitle == null || roomTitle!.isEmpty || roomTitle!.length > 10)
          ? "名稱不得為空或超過10個字元"
          : null;
  ...
  String? startPointValidator(String? value) =>
      startAddress.isEmpty ? "輸入不可為空" : null;
  String? destinationPointValidator(String? value) =>
      destinationAddress.isEmpty ? "輸入不可為空" : null;
